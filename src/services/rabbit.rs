...
impl<E: DbExecutor> BlockchainFetcher<E> {
    pub fn process(&self, blockchain_transaction: BlockchainTransaction) -> impl Future<Item = (), Error = IoError> {
            ...
            self.db_executor
                .execute_transaction(move || -> Result<(), RepoError> {
                    ...
                    if let Some(cr_account) = accounts_repo.get_by_address(blockchain_transaction.to.clone(), AccountKind::Cr)? {
                        if let Some(dr_account) = accounts_repo.get_by_address(blockchain_transaction.to.clone(), AccountKind::Dr)? {
